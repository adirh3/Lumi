...
/// <c>UseBYOKOnly</c> (the BYOK Only block). This is the hard guarantee that no inference request
/// ever reaches GitHub's internal Copilot endpoints when the flag is on. The guarantee is scoped to
/// inference: GitHub sign-in, model discovery, and quota / entitlement checks still occur, but no
/// conversation or inference content is sent to GitHub. Background helper callers (memory agent,
/// coding tool, etc.) catch this and degrade gracefully; the main chat path checks the flag up
/// front and shows a friendly error instead.
/// </summary>
