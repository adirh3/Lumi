...
internal sealed class WindowsCredentialKeyStore : ISecureKeyStore
{
    ...
    public Task DeleteAsync(string key)
    {
        ...
        // ERROR_NOT_FOUND is expected when deleting an absent entry — that is the only failure
        // we tolerate. Every other CredDeleteW failure (permissions, store corruption, policy)
        // must throw, otherwise a failed clear looks successful and orphans the credential.
