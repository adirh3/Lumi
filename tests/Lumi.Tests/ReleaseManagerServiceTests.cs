...
public sealed class ReleaseManagerServiceTests
{
    ...
    public void BuildTools_ExposesDraftOnlyReleaseHelpers()
    {
        ...
        Assert.Contains(tools, tool => tool.Name == "release_import_rema_data");
    ...
    [Fact]
    public void ImportRemaData_PreviewsAndImportsReleaseProfilesOnly()
    {
        var tempRoot = Path.Combine(Path.GetTempPath(), $"lumi-rema-import-test-{Guid.NewGuid():N}");
        Directory.CreateDirectory(tempRoot);
        var dataPath = Path.Combine(tempRoot, "data.json");
        try
        {
                  ...
                  "serviceProfiles": [
                    {
                      ...
                      "repoPath": "C:\\repos\\checkout",
                      "adoProjectName": "Checkout",
                      "pipelines": [
                        { "name": "Checkout-Release", "pipelineId": "42", "stage": "INT", "region": "westus" }
                      ],
                      "healthQueries": [
                        { "name": "Canary health", "query": "requests | count", "dashboardUrl": "https://dashboard" }
                      ],
                      "dependencies": [
                        { "source": "Checkout", "target": "Payments", "reason": "checkout calls payments" }
                      ]
                    }
                  ],
                  "dashboardLayout": { "ignored": true },
                  "shiftView": { "ignored": true }
                }
                """);
            ...
            var preview = InvokePrivateString(service, "ImportRemaData", dataPath, true);
            Assert.Contains("rema_import: preview", preview);
            Assert.Contains("profiles_found: 1", preview);
            Assert.Empty(data.ReleaseServiceProfiles);

            var imported = InvokePrivateString(service, "ImportRemaData", dataPath, false);
            Assert.Contains("rema_import: imported", imported);
            Assert.Contains("profiles_imported: 1", imported);
            Assert.Contains("dashboard and shift UX are intentionally not imported", imported);

            var profile = Assert.Single(data.ReleaseServiceProfiles);
            Assert.Equal("CheckoutService", profile.ServiceName);
            Assert.Single(profile.Pipelines);
            Assert.Single(profile.HealthQueries);
            Assert.Single(profile.Dependencies);

            var duplicate = InvokePrivateString(service, "ImportRemaData", dataPath, false);
            Assert.Contains("profiles_skipped_as_duplicates: 1", duplicate);
            Assert.Single(data.ReleaseServiceProfiles);
        }
        finally
        {
            if (Directory.Exists(tempRoot))
