...
public sealed class SettingsViewModelCredentialStoreTests
{
    ...
    [Fact]
    public async Task DeleteByokEndpoint_WhenCredentialDeleteFails_PreservesEndpointModelsAndSurfacesError()
    {
        var store = new FakeSecureKeyStore { DeleteException = new InvalidOperationException("store locked") };
        var vm = CreateVm(store);
        vm.AddByokEndpointCommand.Execute(null);
        var endpoint = vm.SelectedByokEndpoint!;
        var key = SecureKeyStoreFactory.EndpointKey(endpoint.Id);
        await store.SetAsync(key, "seeded");
        var model = new ByokModel { EndpointId = endpoint.Id, ModelId = "test-model" };
        vm.ByokModels.Add(model);
        var configurationChangeCount = 0;
        vm.ByokConfigurationChanged += () => configurationChangeCount++;
        ...
        Assert.Contains(endpoint, vm.ByokEndpoints);
        Assert.Contains(model, vm.ByokModels);
        Assert.Same(endpoint, vm.SelectedByokEndpoint);
        Assert.Equal(0, configurationChangeCount);
        Assert.True(vm.IsByokValidationVisible);
        Assert.False(string.IsNullOrEmpty(vm.ByokValidationMessage));
    }
    ...
    [Fact]
    public async Task DeleteByokEndpoint_WhenCredentialStoreUnsupported_DeletesAnyway()
    {
        var store = new FakeSecureKeyStore { IsSupported = false };
        var vm = CreateVm(store);
        vm.AddByokEndpointCommand.Execute(null);
        var endpoint = vm.SelectedByokEndpoint!;
        var model = new ByokModel { EndpointId = endpoint.Id, ModelId = "test-model" };
        ...
        Assert.DoesNotContain(endpoint, vm.ByokEndpoints);
        Assert.DoesNotContain(model, vm.ByokModels);
        Assert.Null(vm.SelectedByokEndpoint);
    }

    [Fact]
    public async Task DeleteByokEndpoint_MissingKey_IsSuccess()
    {
        var store = new FakeSecureKeyStore();
        var vm = CreateVm(store);
        vm.AddByokEndpointCommand.Execute(null);
    ...
    [Fact]
    public async Task SaveByokCredentialStoreKey_WhenWriteFails_PreservesEntryAndShowsNoSuccess()
    {
        // A failed OS write must not look successful: the entry is preserved, a localized
        // validation message is shown, and no configuration-change event fires.
        var store = new FakeSecureKeyStore { SetException = new InvalidOperationException("store unavailable") };
        var vm = CreateVm(store);
        vm.AddByokEndpointCommand.Execute(null);
        var ep = vm.SelectedByokEndpoint!;
        ep.ApiKeyMode = ByokApiKeyMode.CredentialStore;
        vm.ByokCredentialStoreKeyEntry = "secret-that-will-not-persist";
        var configurationChangeCount = 0;
        vm.ByokConfigurationChanged += () => configurationChangeCount++;

        await vm.SaveByokCredentialStoreKeyCommand.ExecuteAsync(null);

        // Entry preserved so the user can retry without retyping.
        Assert.Equal("secret-that-will-not-persist", vm.ByokCredentialStoreKeyEntry);
        Assert.False(vm.HasByokCredentialStoreEntry);
        Assert.Equal(0, configurationChangeCount);
        Assert.True(vm.IsByokValidationVisible);
        Assert.False(string.IsNullOrEmpty(vm.ByokValidationMessage));
    ...
    [Fact]
    public async Task ClearByokCredentialStoreKey_WhenDeleteFails_ShowsErrorAndDoesNotReportCleared()
    {
        // A failed OS delete must surface an error and must not report the entry as cleared or
        // fire a configuration-change event (which would make the failure look like success).
        var store = new FakeSecureKeyStore { DeleteException = new InvalidOperationException("store locked") };
        var vm = CreateVm(store);
        vm.AddByokEndpointCommand.Execute(null);
        var ep = vm.SelectedByokEndpoint!;
        ep.ApiKeyMode = ByokApiKeyMode.CredentialStore;
        var expectedKey = SecureKeyStoreFactory.EndpointKey(ep.Id);
        // Seed an entry so the "clear" path has something that should have been removed.
        await store.SetAsync(expectedKey, "seeded");
        var configurationChangeCount = 0;
        vm.ByokConfigurationChanged += () => configurationChangeCount++;
    ...
    [Fact]
    public async Task ClearByokCredentialStoreKey_MissingKey_IsIdempotentSuccess()
    {
        // Deleting an absent key is the one tolerated failure: no error, normal success path.
        var store = new FakeSecureKeyStore();
        var vm = CreateVm(store);
        vm.AddByokEndpointCommand.Execute(null);
        var ep = vm.SelectedByokEndpoint!;
        ...
        Assert.Equal(1, configurationChangeCount);
        Assert.False(vm.HasByokCredentialStoreEntry);
        Assert.False(vm.IsByokValidationVisible);
    }
}
